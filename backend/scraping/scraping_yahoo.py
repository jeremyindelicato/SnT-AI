...
def get_ticker_details(symbol: str, session=None):
    ...
    try:
        ...
        # --- Extraction COMPLÈTE des indicateurs via fin-streamer ---
        # Mapping étendu avec toutes les données importantes
        streamer_map = {
            # Prix et variations
            ...
            'earningsTimestamp': 'Date des résultats',
            'revenuePerShare': 'Revenu par action',
            ...
            if tag:
                ...
                if field == 'regularMarketTime':
                    try:
                        ...
                        details[label] = datetime.fromtimestamp(ts).strftime('%d/%m/%Y %H:%M')
                    except:
                        details[label] = tag.text.strip()
                # Traitement spécial pour les dates de dividendes
                elif 'Date' in label or 'Timestamp' in label:
                    try:
                        value = tag.get('value', tag.text.strip())
                        if value and value.isdigit():
                            ts = int(value)
                            details[label] = datetime.fromtimestamp(ts).strftime('%d/%m/%Y')
                        else:
                            details[label] = value
                    except:
                        details[label] = tag.text.strip()
                else:
                    # Pour les autres champs, prendre la valeur affichée
                    details[label] = tag.text.strip()
            else:
                # Ne pas polluer avec des "n/a" pour les champs optionnels
                pass
